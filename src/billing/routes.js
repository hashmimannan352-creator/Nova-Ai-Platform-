...
// BUG FIX: index.js falls back to Railway's public domain when BASE_URL
// isn't explicitly set, but this file was reading process.env.BASE_URL
// directly with no fallback — if BASE_URL was ever missing on Railway,
// Stripe checkout/portal would redirect to "undefined/?checkout=success".
// Mirrors the exact same fallback logic as ENV.BASE_URL in index.js.
const BASE_URL = (process.env.BASE_URL || (process.env.RAILWAY_PUBLIC_DOMAIN ? `https://${process.env.RAILWAY_PUBLIC_DOMAIN}` : 'http://localhost:5000'));
...
router.post('/checkout', requireAuth, async (req, res) => {
  ...
  try {
    const session = await billing.createCheckoutSession({
      ...
      cancelUrl: `${BASE_URL}/?checkout=cancelled`,
    });
...
function mountWebhook(app) {
  app.post('/api/billing/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
    ...
    try {
      ...
      logger.warn('billing.webhook.signature_invalid', { error: err.message, requestId: req.id });
      return res.status(400).send(`Webhook Error: ${err.message}`);
    }

    try {
      await billing.handleWebhookEvent(event);
      logger.info('billing.webhook.handled', { eventType: event.type, eventId: event.id, requestId: req.id });
      res.json({ received: true });
    } catch (err) {
      logger.error('billing.webhook.failed', { eventType: event?.type, eventId: event?.id, error: err.message, requestId: req.id });
